...
static const char * TAG = "MEDIA-HID";
...
esp_err_t media_hid_init(void)
{
    // Initialize Bluetooth Low Energy (BLE) GAP advertising with the specified appearance and device name
    esp_err_t ret = esp_hid_ble_gap_adv_init(ESP_HID_APPEARANCE_GENERIC, ble_hid_config.device_name);
    ESP_ERROR_CHECK(ret);

    // Register the GATT server callback for handling GATT events
    if ((ret = esp_ble_gatts_register_callback(esp_hidd_gatts_event_handler)) != ESP_OK) {
        ESP_LOGE(TAG, "GATTS register callback failed: %d", ret);
        return ESP_FAIL;
    }

    // Initialize the BLE HID device with the provided configuration, transport type, event callback,
    // and store the HID device pointer in s_ble_hid_param.hid_dev
